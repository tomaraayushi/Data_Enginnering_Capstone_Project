...
def deftunes_pipeline() -> DAG:
    """
    This DAG will run the DefTunes pipeline.
    It will extract data from the Users and Sessions endpoints, transform the data, and perform data quality checks.
    """

    # `start` task based on a `DummyOperator`
    start = DummyOperator(task_id="start")

    # Use a `GlueJobOperator` to create the `api_users_extract_glue_job` task.
    # This task is in charge of creating a Glue job that will ingest the Users endpoint and save the data in JSON format in the landing zone.
